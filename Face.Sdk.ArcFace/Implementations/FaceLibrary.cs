...
namespace Face.Sdk.ArcFace.Implementations
{
    ...
    public partial class ArcFaceHandler
    {
        ...
        /// 人脸库
        /// </summary>
        private readonly ConcurrentDictionary<string, ConcurrentDictionary<string, FaceData>> _faceLibraries =
            new ConcurrentDictionary<string, ConcurrentDictionary<string, FaceData>>() { ["default"] = new ConcurrentDictionary<string, FaceData>() };
        ...
        public async Task InitFaceLibraryAsync(IEnumerable<FaceData> faces, string libraryKey = "default") =>
            await Task.Run(() => _faceLibraries.GetLibrary(libraryKey).InitFaceLibrary(faces));


        public async Task<(bool Success, int SuccessCount)> TryInitFaceLibraryAsync(IEnumerable<FaceData> faces,
            string libraryKey = "default") =>
        ...
        public async Task AddFaceAsync(string libraryKey = "default", params FaceData[] faces) =>
            await Task.Run(() => _faceLibraries.GetLibrary(libraryKey).AddFace(faces));

        public async Task<(bool Success, int SuccessCount)>
            TryAddFaceAsync(string libraryKey = "default", params FaceData[] faces) =>
        ...
        public async Task<int> RemoveFaceAsync(string libraryKey = "default", params string[] faceIds) =>
            await Task.Run(() =>
            {
                ...
                foreach (var faceId in faceIds)
                {
                    ...
                    _faceLibraries.GetLibrary(libraryKey).TryRemove(faceId, out var face);
                    face.Dispose();
                    cnt++;
                }
        ...
        public async Task<OperationResult<Recognitions>>
            SearchFaceAsync(string image, Predicate<FaceData> predicate = null, string libraryKey = "default") =>
        ...
        public async Task<OperationResult<Recognitions>> SearchFaceAsync(string image, float minSimilarity,
            Predicate<FaceData> predicate = null, string libraryKey = "default")
        {
        ...
        public async Task<OperationResult<Recognitions>>
            SearchFaceAsync(Stream image, Predicate<FaceData> predicate = null, string libraryKey = "default") =>
            await SearchFaceAsync(image, _options.MinSimilarity, predicate, libraryKey);
        ...
        private Task SearchFaceAsync(IntPtr featureInfo, IEnumerable<KeyValuePair<string, FaceData>> library,
            ...
            Task.Run(() =>
            {
                ...
                try
                {
                    ...
                    {
                        var similarity = 0f;
                        var code = AsfHelper.ASFFaceFeatureCompare(engine, featureInfo, item.Value.Feature,
                            ref similarity);
                        if (code != 0)
                            continue;
                        ...
                        recognitions.Add(new Recognition(item.Key, similarity));
                    }
                }
                finally
                {
